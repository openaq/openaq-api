import { find, filter, pick, isNumber, sortBy } from 'lodash';
import { db } from '../api/services/db';

export const reconcileLocationIds = async function (athenaQueryResults) {
  // Set correct types of coordinates for incoming locations
  let incomingLocations = athenaQueryResults.map(l => {
    const { lon, lat } = l;
  ...
  for (const incomingLocation of incomingLocations) {
    ...
    if (!currentLocation) {
      ...
      updatedListOfLocations.push(currentLocation);
    }
  }
