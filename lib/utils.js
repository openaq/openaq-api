...
export function queryFromParameters (payload) {
  ...
  //
  // Geo, only acceptable entry is with both coordinates and radius
  //
  if (has(payload, 'coordinates') && has(payload, 'radius')) {
  ...
  } else if (has(payload, 'coordinates')) {
    payload = omit(payload, ['coordinates']);
  } else if (has(payload, 'radius')) {
    payload = omit(payload, ['radius']);
  }
...
/**
 * Make sure the geo payload we're getting is acceptable
 *
 */
export function isGeoPayloadOK (payload) {
  ...
  if (split.length === 2 && split[0] !== '' && split[1] !== '' &&
    ...
    isFinite(Number(payload.radius))) {
    return true;
  }
